...
public interface IUserDataService
{
    ...
    Task<ApiResponse<IEnumerable<KeyValuePair<int, string>>>> GetSubstitutesByEmp(int userId);
    Task<ApiResponse<bool>> UpdateSubstitutes(List<Substitute> substitutes, int userId);
    Task<ApiResponse<LoginResponse>> VerifyLogin(Login login);
    Task<ApiResponse<LoginResponse>> VerifyChangePassword(ChangePassword login);
    Task<ApiResponse<bool>> UpdatePassword(ChangePassword changePassword);
}