...
namespace Timesheet.Client.Services.Implementations
{
    public class TimesheetControlDataService : ITimesheetControlDataService
    {
        ...
        public async Task<ApiResponse<TimesheetControl>> GetTimesheetControlRecord(DateTime period)
        {
            (ResponseStatus status, string data, List<string> errors) = await _baseService.GetList($"{Constants.timesheetControlEndpoint}/{period.Date:yyyy-MM-dd}");
