...
public class UserDataService : IUserDataService
{
    ...
    public async Task<ApiResponse<IEnumerable<KeyValuePair<int, string>>>> GetSubstitutesByEmp(int userId)
    {
        (ResponseStatus status, string data, List<string> errors) = await _baseService.GetList($"{Constants.userEndpoint}/{userId}/empsubstitute");

        return new ApiResponse<IEnumerable<KeyValuePair<int, string>>>
        {
            Status = status,
            Errors = errors,
            Data = status == ResponseStatus.Success
                ? JsonConvert.DeserializeObject<List<KeyValuePair<int, string>>>(data)
    ...
    public async Task<ApiResponse<LoginResponse>> VerifyLogin(Login login)
    {
        (ResponseStatus status, string data, List<string> errors) = await _baseService.Create($"{Constants.loginEndpoint}/verifylogin" , login, false);
        return new ApiResponse<LoginResponse>
        {
            Status = status,
            Errors = errors,
            Data = status == ResponseStatus.Success 
                ?JsonConvert.DeserializeObject<LoginResponse>(data)
                : new LoginResponse()
        };
    }

    public async Task<ApiResponse<LoginResponse>> VerifyChangePassword(ChangePassword login)
    {
        (ResponseStatus status, string data, List<string> errors) = await _baseService.Create($"{Constants.loginEndpoint}/verifychangepassword", login);
        return new ApiResponse<LoginResponse>
        {
            Status = status,
            Errors = errors,
    ...
    public async Task<ApiResponse<bool>> UpdatePassword(ChangePassword changePassword)
    {
        (ResponseStatus status, string data, List<string> errors) = await _baseService.Update($"{Constants.loginEndpoint}/password", changePassword);
