...
namespace Timesheet.Client.Services.Implementations
{
    public class ApprovalDataService : IApprovalDataService
    {
        ...
        {
            (ResponseStatus status, string data, List<string> errors) = await _baseService.Create($"{Constants.approvalEndpoint}/timeoff", new { StartDate = startDate, EndDate = endDate, IsSupervisor = isSupervisor });
        ...
        {
            (ResponseStatus status, string data, List<string> errors) = await _baseService.Update($"{Constants.approvalEndpoint}/timeoff/rejectselect", ids.ToArray());
            return new ApiResponse<bool>
            {
                Status = status,
                Errors = errors,
        ...
        public async Task<ApiResponse<bool>> RejectTimesheetsRequests(List<int> ids)
        {
