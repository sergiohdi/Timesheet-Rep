...
{
    public static DateTime GetPeriod(int year, int month, int period) => period == 2 ? new DateTime(year, month, 16) : new DateTime(year, month, 1);

    public static DateTime GetPeriodStartDate(int year, int month, int day) => day > 15 ? new DateTime(year, month, 16) : new DateTime(year, month, 1);

    public static DateTime GetPeriodStartDate(DateTime date) => date.Day > 15 ? new DateTime(date.Year, date.Month, 16) : new DateTime(date.Year, date.Month, 1);
    ...
    public static string GetDatePeriod(DateTime date) => date.Day > 15 ? "2-2" : "1-2";

    public static long GetPeriodInTicks(int year, int month, int day) => 
        new DateTime(year, month, day,0,0,0,DateTimeKind.Local).Ticks;
